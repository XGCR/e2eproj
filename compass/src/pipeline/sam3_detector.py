...
class SAM3Detector:
    ...
    def segment(self, image, image_path, output_sam3_json_folder_str):
        ...
        # sign
        output_sign = self.processor.set_text_prompt(state=inference_state, prompt=prompt_sign)
        masks_sign, boxes_sign, scores_sign = \
            output_sign["masks"], output_sign["boxes"], output_sign["scores"]

        # arrow
        output_arrow = self.processor.set_text_prompt(state=inference_state, prompt=prompt_arrow)
